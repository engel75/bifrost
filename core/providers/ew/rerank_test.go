...
func TestRerankToEWRerankRequestNil(t *testing.T) {
	req := ToEWRerankRequest(nil)
	assert.Nil(t, req)
}

func TestRerankToEWRerankRequest(t *testing.T) {
	topN := 2
	maxTokens := 128
	priority := 5

	req := ToEWRerankRequest(&schemas.BifrostRerankRequest{
...
func TestRerankParseEWUsageZeroUsage(t *testing.T) {
	usage, ok := parseEWUsage(map[string]interface{}{})
	assert.False(t, ok)
	assert.Nil(t, usage)
}